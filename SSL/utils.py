...
class SSLDataset(Dataset):
    ...
    def __init__(self,
        ...
        # -> 데이터 편향을 아예 없애기 위함임.
        self.data_len_per_class = min([len(os.listdir(os.path.join(self.dir_, cls))) for cls in self.classes])

        # class를 돌면서
        for cls in self.classes:
            paths = glob(os.path.join(self.dir_, cls, "*.jpg")) # 각 클래스의 이미지 경로들을
            shuffle(paths) # 섞어줌

            for i in range(len(paths)):

                if i < self.data_len_per_class: # 편향되지 않게 데이터 개수를 고려하여 selected_data 변수에 추가하고
                    self.selected_data.append((paths[i], int(cls)))

                else: # 그 외는 사용하지 않는 데이터 변수에 추가함
                    self.unselected_data.append((paths[i], int(cls)))
